...
class Feeder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #         subject_id = self.imagepaths[index][0].split('ac/')[-1][0:5]
            ...
            subject_id = self.imagepaths[index][0].split('image/')[-1][0:5]

        image = []
        for i in range(len(self.imagepaths[index])):
            #print(self.imagepaths[index][i])
            if os.path.exists(self.imagepaths[index][i]):
                img = cv2.imread(self.imagepaths[index][i])
                # cv2.imshow(img)
